        ...
        <li class="berocket_term_parent_<?php echo @$term->parent; ?> berocket_term_depth_<?php echo $parent_count; ?> <?php if ( $is_child_parent ) echo 'R__class__R '; ?><?php if( @ $hide_o_value && isset($term->count) && $term->count == 0 && ( !$is_child_parent || !$is_first ) ) { echo 'berocket_hide_o_value '; $hiden_value = true; }  if( @ $hide_sel_value && @ br_is_term_selected( $term, true, $is_child_parent_or, $child_parent_depth ) != '' ) { echo 'berocket_hide_sel_value'; $hiden_value = true; } if( @ $hide_child_attributes && @ $term->parent && in_array(@ $term->parent, $added_categories) ) { echo ' berocket_hide_child_attributes '; } ?>">
            <span>
                <input id='checkbox_<?php echo @ str_replace ( '*' , '-' , $term->term_id), @ str_replace ( '*' , '-' , $term->taxonomy) ?>_<?php echo @ $random_name ?>'
                       class="<?php echo @ $uo['class']['checkbox_radio'] ?> checkbox_<?php echo @ str_replace ( '*' , '-' , $term->term_id), @ str_replace ( '*' , '-' , $term->taxonomy) ?>"
                       type='checkbox' 
                ...
                <label data-for='checkbox_<?php echo @ str_replace ( '*' , '-' , $term->term_id), @ str_replace ( '*' , '-' , $term->taxonomy) ?>' style="<?php echo @ $uo['style']['label']?>"
